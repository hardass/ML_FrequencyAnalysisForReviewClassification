...
var testingFileName string = "test-release.txt"
var outputfilepath string = "test-release.txt"
...
func main() {
	...
	OutputTestResult(outputfilepath)

	fmt.Println("Program End...")
}

func OutputTestResult(outputfilepath string) {
	if _, err := os.Stat(outputfilepath); err == nil {
		err := os.Remove(outputfilepath)
		Check(err, "there is an output file cannot be over written")
	}

	f, err := os.Create(outputfilepath)
	Check(err, "output test result file cannot be created")
	defer f.Close()

	for _, v := range testingParagraph_array {
		content := strconv.Itoa(v.RankPrediction) + " "
		for _, vv := range v.Sentences {
			content = content + vv
		}
		content = content + "\n"
		_, err := f.WriteString(content)
		Check(err, "failed to write line into ouput test result file")
	}
